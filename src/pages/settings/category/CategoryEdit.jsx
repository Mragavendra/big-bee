...
const CategoryEdit = () => {
  ...
  const [category, setCategory] = useState({
    stage: 'Prospect',
    description: 'Initial contact made, needs qualification',
    isActive: true
  });
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            <div className="flex items-center">
              ...
              <div 
                className={`relative inline-flex h-8 w-14 items-center rounded-full cursor-pointer transition-colors ${category.isActive ? 'bg-orange-500' : 'bg-gray-200'}`}
                onClick={toggleActiveStatus}
              >
                <div className={`inline-block h-6 w-6 transform rounded-full bg-white transition-transform ${category.isActive ? 'translate-x-7' : 'translate-x-1'} shadow-sm`}>
                  {category.isActive && (

...
const DesignationEdit = () => {
  ...
  useEffect(() => {
    const fetchDesignation = async () => {
      setFetching(true);
      try {
        const response = await axios.get(`http://localhost:5000/api/designations/${id}`);
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await axios.put(`http://localhost:5000/api/designations/${id}`, {
        name,
        level,
        description,
        is_active: isActive,
  ...
  if (fetching) {
    return (
      <div className="p-6 flex justify-center items-center h-64">
        <div className="text-gray-600">Loading designation details...</div>
      </div>
    );
  }
